...
직접 접근할 수 있지만 특별한 의미론을 갖는 예약된 데이터베이스 이름도 있다.
...
“$where”절을 사용하면 임의의 자바스크립트를 쿼리의 일부분으로 실행해 거의 모든 쿼리를 표현할 수 있다.

따라서 보안상의 이유로 “$where”절 사용을 제한해야 한다.

일반 쿼리보다 더 느리고 인덱스를 사용할 수도 없기 때문에 다른 방법이 없는 경우에만 사용하자.

## 4.5 커서

데이터베이스는 커서를 사용해 find의 결과를 반환한다.

셸에서 커서를 생성하려면 컬렉션에 도큐먼트를 집어넣고 쿼리한 후 결과를 지역변수에 할당한다.

그러면 결과를 한번에 하나씩 볼 수 있다는 장점이 있다.
...
결과를 얻으려면 next 메서드를 사용하고, 다른 결과가 있는지 확인하려면 hasNext를 사용한다.
...
또한 cursor 클래스는 자바스크립트의 반복자 인터페이스를 구현했기 땜누에 forEach 반복문에 사용할 수도 있다.
...
skip 은 개수만큼 건너뛴 결과를 얻는다.

```jsx
> db.c.find().skip(3)
```

sort는 객체를 매개변수로 받는다. 매개변수는 키/값 쌍의 셋, 키는 키의 이름, 값은 정렬 방향이다.
...
몽고DB에는 데이터형을 비교하는 위계 구조가 있다.

최솟값, null, 숫자, 문자열, 객체/도큐먼트, 배열, 이진 데이터, 객체 ID, 불리언, 날짜, 타임스탬프, 정규 표현식, 최댓값

### 4.5.2 많은 수의 건너뛰기 피하기

도큐먼트 수가 적을 때는 skip을 사용해도 문제 없지만, skip은 생략된 결과물을 모두 찾아 폐기하므로 결과가 많이 느려진다.
...
서버 커서를 종료하는 조건. 첫 번째, 조건에 일치하는 결과를 모두 살펴본 후 스스로 정리. 두 번째, 커서가 클라이언트측에서 유효 영역을 벗어나면 드라이버는 데이터베이스에 메시지를 보내 커서를 종료. 세 번째, 사용자가 아직 결과를 다 살펴보지 않았고, 커서가 여전히 유효 영역 내에 있더라도 10분 동안 활동이 없으면 데이터베이스 커서는 자동으로 죽는다.